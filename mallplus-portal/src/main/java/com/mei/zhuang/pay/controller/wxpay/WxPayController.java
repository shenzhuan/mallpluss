package com.mei.zhuang.pay.controller.wxpay;
...
import com.mei.zhuang.oms.service.IOmsPaymentsService;
import com.mei.zhuang.pay.entity.H5ScencInfo;
import com.mei.zhuang.pay.entity.WxPayBean;
...
public class WxPayController extends WxPayApiController {
    ...
    public void wapPay(HttpServletRequest request, HttpServletResponse response,
        ...
        Map<String, String> params = WxPayApiConfigKit
                ...
                .setAttach("IJPay H5支付测试  -By Javen")
                .setBody("IJPay H5支付测试  -By Javen")
                .setSpbillCreateIp(ip).setTotalFee(orderInfo.getPayAmount().multiply(new BigDecimal(100)).toString())
                .setTradeType(TradeType.MWEB)
                .setNotifyUrl(notify_url)
                .setOutTradeNo(String.valueOf(System.currentTimeMillis()))
                .setSceneInfo(h5_info.toString())
    ...
    @ResponseBody
    public Object webPay(HttpServletRequest request, HttpServletResponse response,
                             @RequestParam(value = "orderId", required = false, defaultValue = "0") Long orderId) {
        OmsOrder orderInfo = orderService.getById(orderId);
        ...
        Map<String, String> params = WxPayApiConfigKit
                ...
                .setTotalFee(orderInfo.getPayAmount().multiply(new BigDecimal(100)).toString())
        ...
        if (!PaymentKit.codeIsOK(return_code)) {
            return new CommonResult().failed(return_msg);
        }
    ...
    public AjaxResult micropay(HttpServletRequest request, HttpServletResponse response,
                               @RequestParam(value = "orderId", required = false, defaultValue = "0") Long orderId,
                               @RequestParam(value = "auth_code", required = false, defaultValue = "0") String auth_code) {
    ...
    @ResponseBody
    public AjaxResult appPay(HttpServletRequest request, @RequestParam(value = "orderId", required = false, defaultValue = "0") Long orderId) {
        OmsOrder orderInfo = orderService.getById(orderId);
        // 不用设置授权目录域名
