...
public class GoodsSample extends Model<GoodsSample> {
    ...
    private Integer salesCount;//商品已出售数量
    ...
    private String stockCnf;//减库存方式 0 拍下减库存 1 付款减库存 2 永不减库存
    ...
    private String letterFree;//刻字服务限时免费（1-是，2-否）
    @TableField("letter_status")
    private boolean letterStatus;//刻字服务状态（1-有，2-没有）
    @TableField("letter_time")
    private String letterTime;//刻字服务限时免费时间
    @TableField("card_type")
    private String cardType;//卡片类型（1-全部卡片，2-制定卡片）
    @TableField("card_content")
    private String cardContent;//卡片内容或者id值
    @TableField("card_status")
    private boolean cardStatus;//卡片状态值（1-有，2-没有）
    @TableField("packet_status")
    private boolean packetStatus;//封套状态值（1-有，2-没有）
